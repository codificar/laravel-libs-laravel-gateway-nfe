...
export default {
  props: ["Enviroment", "ProviderId", "Company"],
  data() {
    return {
      //tipoAutenticacao
  ...
  methods: {
    ...
    async companyCertifie() {
      try {
        ...
        formData.append('provider_id', this.certifie.provider_id);
        const {data} = await axios.post("/admin/provider/company/certified", formData);       
        if (data.sucess) {
          this.$swal({
            title: data.data,
            type: "success",
          });
          // window.location.href = '/admin/provider/company/create/'+this.certifie.provider_id;
        } else {
          this.$swal({
   ...
   async loginCompany() {
      try {       
        ...
        if (data.sucess) {
          ...
          window.location.href = '/admin/provider/company/create/'+this.ProviderId;
        } else {
          this.$swal({
            title: "Valores invalidos",
            html: data.data,
    ...
    async getAuthType() {
      try {
        const { data } = await axios.post("/admin/provider/company/auth", {
          ibgeCode: this.company.address.ibgeCode
        });
        console.log("data", data);        
        // this.company.tipoAutenticacao = data.data.tipoAutenticacao 
        // this.company.tipoAssinaturaDigital = data.data.assinaturaDigital             
